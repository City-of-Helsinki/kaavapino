...
class TestVisibilityEdgeCases:
    """
    Adversarial tests for visibility logic edge cases.
    """
    
    def test_vis_bool_string_true_treated_correctly(self):
        """
        CATCHES BUG: String "True" treated as boolean True.
        
        Frontend might send string instead of boolean.
        """
        value = "True"  # String, not boolean
        
        # The visibility check should handle this correctly
        # (not crash, and ideally treat as truthy)
        is_explicitly_false = value is False  # Python identity check
        
        assert is_explicitly_false is False, (
            "String 'True' should not be treated as False"
        )
    
    def test_vis_bool_none_defaults_to_visible(self):
        """
        CATCHES BUG: Missing vis_bool causes deadline to be hidden.
        
        When vis_bool is not in attribute_data, deadline should be visible.
        """
        attribute_data = {}  # No vis_bool set
        
        vis_value = attribute_data.get('jarjestetaan_oas_esillaolo_1')
        
        # None should not be treated as explicitly False
        is_hidden = vis_value is False
        
        assert is_hidden is False, (
            f"Missing vis_bool (None) should not hide deadline. "
            f"Value: {vis_value}, is_hidden: {is_hidden}"
        )
    
    def test_vis_bool_zero_treated_as_false(self):
        """
        CATCHES BUG: Integer 0 not treated as false.
        
        Database might store 0 instead of False.
        """
        value = 0
        
        # Python treats 0 as falsy
        is_falsy = not value
        
        assert is_falsy is True, (
            "Integer 0 should be treated as falsy"
        )
