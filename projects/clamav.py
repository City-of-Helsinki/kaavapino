...
class ClamavClient:
    ...
    def __init__(self):
        if not settings.CLAMAV_URL:
            log.warning("CLAMAV_URL not defined in settings -- Clamav Client not enabled")
            return
    ...
    def scan(self, name, _bytes):
        ...
        response = requests.post(self.BASE_URL + "scan", files={'FILES': (name, _bytes)})

        if not response or response.status_code != 200 or not response.json()["success"]:
