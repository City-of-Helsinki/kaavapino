...
for row in reader:
    ...
    for column_num, attribute_name in date_column_map.items():
        ...
        date_values[pr_id][attribute_name] = datetime.strptime(
            row[column_num], "%d.%m.%Y"
        ).date()

    if row[10]:
        try:
            k_date = datetime.strptime(row[10], "%d.%m.%Y").date()
            date_values[pr_id]["Käynnistys"] = k_date - timedelta(
                days=randint(30 * 3, 30 * 5)
            )
    ...
    if "Käynnistys" not in date_values[pr_id]:
        ...
        if matches:
            date_values[pr_id]["Käynnistys"] = date(
                day=1, month=1, year=int(matches.group(1))
            )
...
for feat in json_dict["features"]:
    ...
    try:
        ...
        print(
            "Geometry for planning project id " + pr_id + " missing, omitting project"
        )
        continue
    if not coord:
...
strategy_choices = set(
    attr_values["PROJECT_TAVOITE"]
    + attr_values["PROJECT_TAVOITE_2"]
    + attr_values["PROJECT_TAVOITE_3"]
)
...
strategy_choices = sorted(
    [x for x in strategy_choices if x and x != "None"],
    key=lambda x: (int(x.split(".")[0]), int(x.split(".")[1].split(" ")[0])),
)
strategy_choice_objs = {}
for idx, c in enumerate(strategy_choices):
    obj = AttributeValueChoice(
        identifier=create_identifier(c), value=c, index=idx, attribute=attr_obj
    )
    obj.save()
...
for project in projects.values():
    ...
    strategy = (
        project.get("PROJECT_TAVOITE"),
        project.get("PROJECT_TAVOITE_2"),
        project.get("PROJECT_TAVOITE_3"),
    )
